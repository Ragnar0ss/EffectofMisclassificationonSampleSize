SampleSizetoPower1sampleFixSeSp <- function(p_null, 
                                  alt = "two.sided", 
                                  met = "Clopper-Pearson", 
                                  alpha = 0.05, 
                                  pwr = 0.8, 
  ...
  # Function to calculate the required sample size for a binomial test when both Sensitivity and Specificity are considered known fixed parameters.
  #  
  # Args:
  ...
  BinomialPowerFixSeSp <- function(n,
                                   p_null,
                                   p_alt,
                                   conf.level = 0.95,
                                   alt = "two.sided",
    ...
    # Function to calculate the power of a binomial test when Se and Sp are fixed (known values).
    #  
    # Args:
  ...
  if (alt == "two.sided") {
      if (p_null == p_alt) {
        ...
        repeat{
          ntopwr <- BinomialPowerFixSeSp(n = val, p_null = p_null, p_alt = p_alt, 
...
SampleSizetoPower1sampleFixSeSp(p_null= 0.01, 
                      alt = "t", 
                      met = "Cl", 
                      alpha = 0.05, 
                      max_alpha=.06, 
...
p_null=c(.01,.02,.03,.05,.10,.20,.30)
p_alt=c(.10,.12,.13,.15,.20,.35,.50)
for (i in 1:length(p_null)) for (Se in c(1,.99,.98,.95, 0.9)) for (Sp in c(1,.99,.98,.95, 0.9))
      cat("p_null:",p_null[i],"p_alt:",p_alt[i],"Se:",Se,"Sp:",Sp,"Sample Size: ",
          SampleSizetoPower1sampleFixSeSp(p_null = p_null[i], p_alt = p_alt[i],
                                          Se = Se, Sp = Sp, max_n = 1000),"\n", 
          file = "interimresults_Fix-SS.txt", append = TRUE)

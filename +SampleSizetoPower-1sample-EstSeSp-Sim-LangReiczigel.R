...
SampleSizetoPower1sampleEstSeSp(p_null = 0.01,
                                 alpha = 0.05, 
                                 pwr = 0.8, 
                                 p_alt = 0.1, 
                                 min_n = 20, 
...
for (i in 1:length(p_null))
  for (Se_exp in c(1,.99,.98,.95, 0.9)) for (Sp_exp in c(1,.99,.98,.95, 0.9))
    for (n_se in c(25, 50, 100, 500, 1000)) for (n_sp in c(25, 50, 100, 500, 1000))
      cat("p_null:",p_null[i],"p_alt:",p_alt[i],"Se:",Se_exp,"Sp:",Sp_exp,"n_se:",n_se,"n_sp:",n_sp,
          "Sample Size: ",SampleSizetoPower1sampleEstSeSp(p_null = p_null[i], p_alt = p_alt[i], 
                                                          Se_exp = Se_exp, Sp_exp = Sp_exp, 
                                                          n_se = n_se, n_sp = n_sp, max_n = 5000),"\n", 
          file = "interimresults_EstSeSp-LangReizigel-SS.txt", append = TRUE)
